...
# Load Data
data_dir = '../mr_data/mrf_2d/data/'
ksp_cal = np.load(data_dir + 'ksp_cal.npy')
mps = np.load(data_dir + 'mps.npy')
im_size = mps.shape[1:]
ndim = len(im_size)
n_coil = ksp_cal.shape[0]
calib_size = ksp_cal.shape[1:]

if coil_compress:
    sub, ksp_cal, mps = calc_coil_subspace(ksp_cal, 12, ksp_cal, mps)
...
img_cal = sp.ifft(ksp_cal, axes=range(-ndim, 0))
source_vecs = gen_source_vectors_rot(num_kerns=10,
                                     num_inputs=5,
                                     ofs=0.15,
                                     line_width=2.0)
